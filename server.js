...
/* Set up a static web-server that will deliver filse from the filesystem */
...
/* payload: 
	  		{
	 		'requested_user': the socket id of the person to be play with
			}
			game_start_response:
			{
				'result': 'success',
	...
	*/
	...
	socket.on('play_token', function(payload){
		...
		/* Check that the the player has previously registered */
		...
		if(('undefined' === typeof row) || row <0 || row > 7){
			var error_message = 'play_token didn\'t specify a valid row and aborted';
			log(error_message);
			socket.emit('play_token_response', 	{
													result: 'fail',
													message: error_message
		...
		if(('undefined' === typeof column) || column <0 || column > 7){
			var error_message = 'play_token didn\'t specify a valid column and aborted';
			log(error_message);
			socket.emit('play_token_response', 	{
													result: 'fail',
													message: error_message
		...
		if(('undefined' === typeof color) || !color || (color != 'blue' && color != 'red')){
...
while((numClients-1) > 2);
	...
	if((games[game_id].player_blue.socket != socket.id) && (games[game_id].player_red.socket != socket.id)){
		console.log('Player isn\'t assigned a color: '+socket.id);
		/* and if there isn't a color to give them */ 
		if((games[game_id].player_red.socket != '') && (games[game_id].player_blue.socket != '')){
			games[game_id].player_blue.socket = '';
			games[game_id].player_blue.socket.username = '';
			games[game_id].player_red.socket = '';
			games[game_id].player_red.socket.username = '';
		}
	}
	...
	if(games[game_id].player_blue.socket == ''){ 
		if(games[game_id].player_red.socket != socket.id){
			...
			games[game_id].player_blue.socket.username = players[socket.id].username;
		}
	}
	if(games[game_id].player_red.socket == ''){ 
		if(games[game_id].player_blue.socket != socket.id){
			games[game_id].player_red.socket = socket.id;
			games[game_id].player_red.socket.username = players[socket.id].username;
		}
	}
	...
	if(count == 64){
		...
		/* Delete old games aftrer 1 hour */
